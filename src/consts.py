...
USER_MUST_SET_CITY_OR_AGE_MESSAGE = '''Упс... Чтобы использовать Upleisure нужно указать в профиле
...
DID_NOT_GET_IT_MESSAGE = 'Извиняюсь, не понял вас 🙄'

SEND_EVENT_MESSAGE = '''Окей, отправь мне описание и фоточки.
Отправь 0(нулик) для отмены.
P.S.
все заявки действуют в течение пяти суток 🕗'''

SEND_FEEDBACK_MESSAGE = '''Окей, скажи мне, что тебе не нравится в Upleisure
 и что можно улучшить. Отправь 0(нулик) для отмены.'''

FEEDBACK_TOO_SMALL_MESSAGE = '''Твой фидбэк был слишком короткий :('''

EVENT_TOO_SMALL_MESSAGE = '''Твоя заявка слишком коротка :('''
